...
class QCalendarPicker: UIView, UIGestureRecognizerDelegate, CalendarViewDelegate, TimePickerViewDelegate {
    ...
    private lazy var calendarView: CalendarView! = {
        var calendarViewH = self.baseView.height - self.dateBtn.height
        if screenHeight == 812 {
            calendarViewH = self.baseView.height - self.dateBtn.height - 34
        }
        let calendarView = CalendarView(frame: CGRect(x: 0, y: self.dateBtn.height, width: screenWidth, height: calendarViewH))
        calendarView.delegate = self
        return calendarView
    }()
    private lazy var timePickerView: TimePickerView! = {
        let timePickerView = TimePickerView(frame: self.calendarView.frame)
        timePickerView.x = self.baseView.width
